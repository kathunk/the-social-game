...
class TheGreatRealignment extends BaseChallengeClass implements SupportsTeamSwaps
{
    ...
    public function onPlayerJoinedTeam(
        ...
        $team_state->addToScoreHistory(
            icon: '👋',
            points: $points,
            description: $player_state->name.' joined during the Great Realignment',
        );
        $previous_team->addToScoreHistory(
            icon: '👻',
            points: -$points,
            description: $player_state->name.' left during the Great Realignment',
        );
