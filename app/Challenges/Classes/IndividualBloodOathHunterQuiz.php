...
class IndividualBloodOathHunterQuiz extends BaseChallengeClass implements SupportsPeckingOrderBallots
{
    ...
    {
        return [
            'quiz_submissions' => $this->challenge_state->game()->players()->mapWithKeys(fn ($p) => [$p->id => ['guess_player_ids' => null]])->toArray(),
    ...
    public function frontendComponent(Player $player): array
    {
        ...
        $has_guessed = $this->challenge->challenge_data['quiz_submissions'][$player->id]['guess_player_ids'] !== null;
        ...
        return $this->form()
            ...
            ->when(! $has_guessed, fn ($form) => $form->select(
                ...
                ->button(
                    label: 'Submit Guess',
                    action: 'guess',
                    properties_to_validate: ['guess_player_id', 'guess_player_id_2'],
                )
                ->endGroup()
            )
    ...
    public function guess(Player $player, array $params)
    {
        PlayerSubmittedQuizGuess::fire(
            ...
            guess: [
                'guess_player_ids' => [$params['guess_player_id'], $params['guess_player_id_2']],
            ],
        );
    }
    ...
    public function onChallengeEnded(
        ...
        $game_state->players()->each(function ($player) use ($oath_data) {
            ...
            $guessed_player_ids = $guess['guess_player_ids'];
            $guessed_player_1 = PlayerState::load($guessed_player_ids[0]);
            $guessed_player_2 = PlayerState::load($guessed_player_ids[1]);

            $guessed_player_is_in_blood_oath = isset($oath_data['pairs'][$guessed_player_ids[0]])
                && $oath_data['pairs'][$guessed_player_ids[0]] === $guessed_player_ids[1];

            if ($guessed_player_is_in_blood_oath) {
                $player->addToScoreHistory(1, 'Correctly guessed that '.$guessed_player_1->name.' and '.$guessed_player_2->name.' are in a blood oath', true);
                $guessed_player_1->addToScoreHistory(-1, $player->name.' guessed that you are in a blood oath', true);
                $guessed_player_2->addToScoreHistory(-1, $player->name.' guessed that you are in a blood oath', true);
            }
        });
    }
}
