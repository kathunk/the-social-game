...
class IndividualLargestDecreaseQuiz extends BaseChallengeClass implements SupportsPeckingOrderBallots
{
    ...
    const DESCRIPTION = 'Guess which player will have the largest score decrease at the end of this round. If you are correct, you will gain one hidden point, that will not be revealed to your opponents until the end of the game.';
    ...
    public function onChallengeEnded(
        ...
        $game_state->players()->each(function ($player) use ($largest_decrease_ids) {
            ...
            if ($largest_decrease_ids->contains($guess_id)) {
                $player->addToScoreHistory(1, 'Correctly guessed a player with the largest score decrease', true);
            }
        });
    }
}
